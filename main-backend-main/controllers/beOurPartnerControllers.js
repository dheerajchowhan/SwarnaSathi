...
exports.createFormSubmission = async (req, res) => {
  try {
    const { type, name, phone, pincode, email, loanType, password } = req.body;
...
exports.sendOTP = async (req, res) => {
  try {
    ...
    try {
      ...
      // Log for debugging
      console.log(`OTP ${otp} generated for ${phone} (Loan type: ${loanType || 'gold-loan'})`);
      ...
      try {
        ...
        return res.status(200).json({
          success: true,
          message: 'OTP generated successfully but SMS delivery failed. Check console for OTP.',
...
exports.login = async (req, res) => {
  try {
    ...
    const user = await FormSubmission.findOne({ phone }).select('+password');
    if (!user) {
    ...
    if (!user.password) {
      return res.status(401).json({ 
        success: false, 
        message: 'Account not set up with password. Please use OTP login.' 
      });
    }
