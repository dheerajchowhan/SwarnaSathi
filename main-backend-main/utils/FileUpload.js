...
const s3 = new S3Client({
  ...
  credentials: {
    accessKeyId: process.env.AWS_ACCESS_KEY_ID || 'AKIA5WLTSZSBHWL4VIN7',
    secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY || '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',
  },
});
...
const fileFilter = (req, file, cb) => {
  if (req.body.mediaType === 'video') {
    if (allowedVideoTypes.includes(file.mimetype)) {
      ...
      cb(new Error('Invalid video type. Only mp4, webm, and quicktime are allowed.'), false);
    }
  } else {
    if (
...
const upload = multer({
  storage: multerS3({
    s3: s3,
    bucket: process.env.AWS_BUCKET_NAME || 'dhruvacapital',
    acl: 'public-read',
